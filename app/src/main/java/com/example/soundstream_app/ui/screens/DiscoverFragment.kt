...
class DiscoverFragment : Fragment() {
    ...
    private fun setupLists() {
        ...
        binding.rvPopularSingers.layoutManager =
            LinearLayoutManager(requireContext(), LinearLayoutManager.HORIZONTAL, false)
        binding.rvPopularSingers.adapter =
            ArtistAdapter(RawAudioProvider.popularSingers)
    }
    ...
    private fun loadContent() {
        viewLifecycleOwner.lifecycleScope.launch {
            ...
            val songsFromDb = db.songDao().getAllSongs().map { entity ->
                Song(
                    id = entity.id,
                    title = entity.title,
            ...
            madeForYouAdapter.updateItems(
                RawAudioProvider.getRandomMadeForYouPlaylists(allSongs)
            )
        }
    }
    ...
    private suspend fun updateFavoriteIds(db: AppDatabase) {
        ...
        favoriteSongIds =
            if (user != null && !SessionManager.isGuest) {
                db.favoriteDao()
                    .getFavoriteSongIds(user.username)
                    .toSet()
            } else {
                emptySet()
            }
    ...
    private fun playSong(song: Song, queue: List<Song> = allSongs) {
        ...
        refreshMiniPlayer()
        findNavController()
            .navigate(R.id.action_discoverFragment_to_playerDetailFragment)
    }

    private fun showTrendingAlbumSongs(
        playlist: PlaylistWithSongs
    ) {

        val dialogBinding =
            DialogAlbumSongsBinding.inflate(layoutInflater)

        dialogBinding.tvDialogAlbumTitle.text =
        ...
        val adapter = AlbumSongAdapter(songs) { selectedSong ->
            playSong(selectedSong, songs)
            dialog.dismiss()
        }

        dialogBinding.rvAlbumSongs.layoutManager =
    ...
    private fun toggleFavorite(song: Song) {
        ...
        if (user == null || SessionManager.isGuest) {
            Toast.makeText(
                requireContext(),
                "Đăng nhập để yêu thích bài hát",
                Toast.LENGTH_SHORT
            ).show()
            return
        }
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            ...
            if (favoriteSongIds.contains(song.id)) {
                ...
                db.favoriteDao()
                    .removeFavorite(user.username, song.id)
    ...
    private fun showPlaylistPicker(song: Song) {
        ...
        if (user == null || SessionManager.isGuest) {
            Toast.makeText(
                requireContext(),
                "Đăng nhập để thêm vào playlist",
                Toast.LENGTH_SHORT
            ).show()
            return
        }
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            ...
            val playlists =
                db.playlistDao().getPlaylistsForUser(user.username)
            ...
            val titles =
                playlists.map { it.playlist.title }
                    .toTypedArray()
    ...
    private fun setupMiniPlayer() {
        ...
        binding.miniPlayer.btnMiniVolume.setOnClickListener {
            Toast.makeText(
                requireContext(),
                "Chức năng tắt tiếng đang phát triển",
                Toast.LENGTH_SHORT
            ).show()
        }

        binding.miniPlayer.progressMini.progress =
            PlaybackManager.progressPercent
    ...
    private fun refreshMiniPlayer() {
        ...
        if (current != null) {
            ...
            binding.miniPlayer.miniPlayerContainer
                .setOnClickListener {
                    playCurrentMiniSong()
                }
            ...
            binding.miniPlayer.imgMiniSong
                .setImageResource(R.drawable.uth)
